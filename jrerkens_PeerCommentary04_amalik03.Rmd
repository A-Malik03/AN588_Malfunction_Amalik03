...
Hey no worries at all I think you did great! I've added my comments throughout your document, feel free to see what's up.
...
library(curl) # I normally like to put my libs in a separate chunk that way you don't call them everytime you run the chunk you're working on -Jimmy
library(tidyverse)
# library(manipulate), not available for the version of R I'm currently using 2023.06.2+561-Jimmy
...
P = NULL
CI = NULL
z.prop.test <- function(p1, n1, p2 = NULL, n2 = NULL, p0, alternative = c("two.sided", "less", "greater"), conf.level = 0.95){ #  I like the alternative = c("") argument, but i think default is two.sided  -Jimmy

  
  if(is.null(p2) || is.null(n2)){    ##One sample z-test 
    # the above logic statement should be &&-Jimmy
      phat <- mean(p0) # head's up p1 = phat, p0 is what we're testing against-Jimmy
      Z <- (phat-p1) / (sqrt(p1 * (1-p1)/ n1)) # within a function assign variables using "="-Jimmy
    ...
    if(alternative == "two.sided"){
      P = 2 * pnorm(Z, lower.tail = FALSE) # smart to standardize-Jimmy
    }
  #confidence interval for 1 sample test
     lower <- phat - qnorm(1- (1- conf.level)/2) * sqrt(phat * (1-phat)/n1) # should be p1 and not phat, phat is our sample, p0 is the null-Jimmy
     upper <- phat + qnorm(1- (1- conf.level)/2) * sqrt(phat * (1-phat)/n1) 
    CI <- c(lower, upper) 

  if(is.null(p2) == FALSE || is.null(n2) == FALSE){ # needs to be &-Jimmy
  ...
  if((n1 * p1 < 5 || n1 * (1-p1) < 5)){ # solid or statement, what happens when either p2 or n2 isn't defined? -Jimmy
 ...
 ##end check with rules of thumb
  ...
  print(c(Z, P, CI)) # head's up this is a vector, not a list-Jimmy
...
d <- na.omit(d) # ^head's up you don't need to get rid of columns for this -Jimmy
# personally I would use more detailed variable names "d" could be anything -Jimmy
...
linmodel
confint(linmodel, level = 0.90) # note that CI changes for each individual x, check out the confidence band in geom_smooth() output-Jimmy
```
...
df <- cbind.data.frame(d$Brain_Size_Species_Mean, d$MaxLongevity_m, lin.conf.int, lin.prid.int, log.conf.int, log.prid.int ) # ^ goodwork this looks correct-Jimmy
...
logplot
# make sure for geom_line() arguments that x = log(brainsize), otherwise looks good -Jimmy
```
I'm not sure what I'm doing wrong, did I forget a crucial step?

```{r}
point.est <- predict(linmodel, x =800, interval = 'confidence', level = 0.90) #check your "new_data" argument -Jimmy
point.est
